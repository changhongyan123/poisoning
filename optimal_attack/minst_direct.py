...
def SGD(key, weight, grad, lr, grad_norm):
...
def generate_poisoning_fun(index = 0,total_round=5):
    ...
    for j in range(index):
        with mx.Context(dev):
            ...
            label_p = poisoning_l[j]
            # data_p = data
            poisoning_d[j].copyto(data_p)
            ...
            for key in model.arg_dict.keys():
                SGD(key, model.arg_dict[key], model.grad_dict[key], attacked_model_lr,batch_size)
            val_acc = 0
            nbatch = 0
            val_iter.reset()
            for databatch in val_iter:
            ...
            for round in range(total_round):
                ...
                poisoing_data_list.append(data_p.asnumpy())
                poisoing_label_list.append(label_p.asnumpy())
            all_data.append(poisoing_data_list)
            all_label.append(poisoing_label_list)
                # filename = 'poisoning_data/direct_round'+str(round)+'_data'+ str(index) +'_random'
                # # save_poisoning_data(filename,data_p,label_p)
    return all_data,all_label


start_time = time.time()
data,label = generate_poisoning_fun(index=600,total_round=5)
filename = '../poisoningData/gradient_attack_mnist.npz'
save_poisoning_data(filename,data,label)
end_time = time.time()

# print('Finish genertiave 600 using the gradinet ascent method %d' %(end_time-start_time))
